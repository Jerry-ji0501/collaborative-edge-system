...
class LayerKVCache:
    ...
    Stores ``k``/``v`` as ``[B, T, H_local, D]`` and the positions of the cached
    tokens as ``[B, T]``.  Which tokens/heads a rank caches depends on the
    parallel strategy (TP: its heads; ring SP: its tokens; Ulysses SP: its
    heads over all tokens), but attention only relies on the stored positions.
    """
    ...
    def _grow(self, k: torch.Tensor, v: torch.Tensor, pos: torch.Tensor, needed: int) -> None:
        ...
        B = k.shape[0]
        new_k = k.new_empty((B, capacity) + tuple(k.shape[2:]))
        new_v = v.new_empty((B, capacity) + tuple(v.shape[2:]))
    ...
    def append(self, k: torch.Tensor, v: torch.Tensor, pos: torch.Tensor) -> None:
        ...
        self.pos[:, self.length : self.length + n] = pos
        self.length += n

    def view(
        self, like_k: Optional[torch.Tensor] = None, like_pos: Optional[torch.Tensor] = None
    ) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """Cached ``(k, v, pos)``; empty tensors shaped like ``like_*`` if unused."""
        if self.k is None:
        ...
        n = self.length
        return self.k[:, :n], self.v[:, :n], self.pos[:, :n]
